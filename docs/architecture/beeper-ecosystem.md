...
A **Beeper** is a catalog definition (manifest + checker). An **Install** is an account-owned running instance of that Beeper: config, cron, alert state, and default channels. When the alert state machine says to notify, the Install creates a one-shot **Beep**. Beep is only the notification; it never runs a checker.
...
flowchart TD
  ...
  Run --> Job["RunBeeperJob (queue: checks)"]
  Job --> Checker["Beeper::Checkers::*#call(config:)"]
  Checker --> Target["Target URL / TLS endpoint / heartbeat state"]
  Checker --> Eval["AlertEvaluator on the Install"]
...
**2. Probe outcome lives on `beeper_runs`, delivery outcome on `beep_runs`.** `beeper_runs.check_status` (`ok` / `alerting` / `error`) is the checker result. `beep_runs.status` (`pending running succeeded failed …`) means *did we deliver*. A healthy probe that creates no Beep is not a skipped or failed delivery.

**3. Notification is a decision on the Install; a notify creates a new once Beep.** The alert state machine (`AlertEvaluator`) runs against the Install. `should_notify` → `Beeps.create!(kind: once, …)` with channels copied onto the Beep and optional `beeper_install_id` so the inbox can point back at the Install. Delivery then uses the existing Beep path. One notify event → one Beep → N channel types on that Beep (same BeepRun). A 5-minute check does not email every 5 minutes for the whole outage.

**4. Official checkers stay in-process Ruby behind `Beeper::Checkers::*`.** No JS sandbox and no new service in this period. Site Uptime and SSL expiry are small `Net::HTTP` / `OpenSSL` classes. The `Beeper::Checker` interface is the seam: a sandboxed runner later is an implementation change behind it, not a refactor of Install / Beep.
...
| `last_ping_at`           | datetime | Stamped by the ping endpoint, read by the Heartbeat checker.        |
...
| `check_status`       | string   | `ok`, `alerting`, or `error`.                                        |
| `check_result`       | json     | `{ title, message, metrics, log }`. **Capped at 8 KB** before write. |
...
Threshold comes from the manifest (`failure_threshold`, default 2) and is user-overridable. Created Beep copy distinguishes "target is failing" from "the check could not run"; an Install with many consecutive `error` results is surfaced as broken in the UI rather than reported as an outage.
...
```ruby
# Beeper::CheckResult = Data.define(:status, :title, :message, :metrics)
#   status: :ok | :alerting | :error

class Beeper::Checkers::SiteUptime
  def call(config:)
    # → Beeper::CheckResult
  end
end
...
Heartbeat is not an outbound probe. It fits the same interface: the inbound `POST /ping/:token` endpoint only stamps `last_ping_at` on the Install, and the *scheduled* check evaluates staleness locally. That endpoint is public and unauthenticated by nature (that is the product), so it needs rate limiting per token, an opaque high-entropy token, and no response body that leaks account information.

---

## Security

**SSRF is the main exposure** — `target_url` is user-supplied and the Ruby checker fetches it, sandbox or not. Without egress control, checkers become a proxy into our own network and to the cloud metadata endpoint (`169.254.169.254`).
...
A 5-minute check produces ~8,640 `beeper_runs` rows per month **per Install**. Probe volume belongs on `beeper_runs`, not on `beep_runs`. Notification Beeps stay sparse (threshold crossings and recoveries).

Retention, per-account Install quotas, and check concurrency limits are needed before public sign-up; they are non-goals of this split.
...
Add `plugin` or a monitor kind to `Beep#kind`; put alert state, ping token, or check outcome on `beeps` / `beep_runs`; overload `beep_runs.status` with probe outcome; reuse `status: firing` for alert state; run checks on the `default` queue; build a JS sandbox to execute first-party code; ship custom scripts before the isolation requirements above are met.
