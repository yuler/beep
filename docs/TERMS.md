...
| Channel          | One user-owned delivery destination (`channels` row). Adapter `kind`: `cli`, `email`, `web_push`, `webhook` (planned: `ios`, `android`, `desktop`).  |
...
| Webhook Channel  | User-owned; no account-level shared webhook in this period.                                                                                           |
