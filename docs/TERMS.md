...
| Title                         | title         | field `title`          | Required headline; max 80                                                   |
| Body                          | body          | field `body`           | Optional Markdown (bold, italic, links, lists, line breaks); max 2000       |
...
| Run status                    | status (Run)  | field `status`         | `pending` \| `running` \| `succeeded` \| `failed` \| `skipped` \| `expired` |
