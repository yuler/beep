...
- `once`: detail-page **Send now** is a real delivery that consumes the schedule — after success the Beep is `completed` with `next_run_at = nil`. If `run_at` is still in the future the UI confirms this before sending.
- `recurring`: detail-page **Trigger run** is an extra run — the schedule captured before firing is restored on completion, so the next slot is preserved.
