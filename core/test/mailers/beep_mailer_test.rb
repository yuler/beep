...
class BeepMailerTest < ActionMailer::TestCase
  test "beep formats subject with prefix and timestamp and includes card details" do
    ...
    scheduled_for = Time.utc(2026, 9, 20, 0, 25, 0)
    ...
    send_time = Time.utc(2026, 9, 20, 8, 25, 0)
    travel_to send_time do
  ...
  test "beep uses send time not scheduled_for for subject and triggered at" do
    account = accounts(:john_account)
    beep = Beep.create!(
      account: account,
      kind: :once,
    ...
    run = beep.runs.create!(scheduled_for: Time.utc(2026, 9, 20, 4, 0, 0))
    ...
    travel_to send_time do
      ...
      assert_equal "[Beep] Delayed ping (09-20 16:25)", email.subject
      assert_match "2026-09-20 16:25 CST", email.html_part.body.to_s
    end
  end
  ...
  test "beep falls back to beep timezone or UTC when user timezone is blank" do
    ...
    send_time = Time.utc(2026, 9, 20, 16, 25, 0)
    travel_to send_time do
      email = BeepMailer.beep(run, user: user)
