class Api::V1::Cli::Authorizations::TokensController < Api::V1::BaseController
  ...
  def create
    ...
    @auth = Cli::Authorization.find_by(device_code: params[:device_code].to_s.strip)
    unless @auth
  ...
  private
    def device_flow_error(error, description, status: :bad_request)
      ...
      render "api/v1/cli/authorizations/device_flow_error", status: status
    end
