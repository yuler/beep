...
// and has not yet been notified to the user. It marks the version as notified once read.
func CheckNotice(workspaceDir string) string {
	...
	// Only print notification if stderr is an interactive terminal
	...
	// Mark as notified so the user is only alerted once for this version
...
func RunDaemonUpdateProbe(ctx context.Context, ws string, logFn func(format string, args ...any)) {
	...
	checkFn := func() {
		...
		if CompareVersions(rel.Tag, version.Version) > 0 {
			if logFn != nil {
