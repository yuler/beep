...
func newUpgradeCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short:   "Upgrade beep CLI to the latest version or a specific version",
		Long: ui.Bold(ui.Cyan("beep upgrade")) + ` - Self-update the beep CLI binary in-place.

Checks GitHub Releases for new versions, downloads the platform-specific
binary archive, verifies the SHA-256 checksum, and replaces the running executable.`,
...
func runUpgrade(cmd *cobra.Command, args []string) error {
	...
	if flagUpgradeCheck {
		...
		if cmp > 0 {
			fmt.Println()
			fmt.Printf("%s %s\n", ui.Yellow("!"), ui.Bold(fmt.Sprintf("A new release of beep is available: %s → %s", ui.Dim(version.Version), ui.Green(rel.Tag))))
			if rel.HTMLURL != "" {
				fmt.Printf("  %s %s\n", ui.Dim("Release notes:"), ui.Cyan(rel.HTMLURL))
			}
			fmt.Printf("  %s Run %s to update.\n", ui.Dim("Action:       "), ui.Bold(ui.Cyan("beep upgrade")))
		} else if cmp == 0 {
			fmt.Printf("%s beep is already up to date (%s)\n", ui.Green("✓"), ui.Bold(version.Version))
		} else {
	...
	if cmp == 0 && !flagUpgradeForce {
		fmt.Printf("%s beep is already at the target version %s (%s)\n", ui.Green("✓"), ui.Bold(rel.Tag), ui.Dim("use --force to reinstall"))
		return nil
	}
	...
	if ui.IsInteractive() && !flagUpgradeYes && !flagNoInteractive {
		var confirm bool
		actionDesc := fmt.Sprintf("Upgrade beep from %s to %s?", version.Version, rel.Tag)
		if cmp < 0 {
			actionDesc = fmt.Sprintf("Downgrade beep from %s to %s?", version.Version, rel.Tag)
		} else if cmp == 0 {
			actionDesc = fmt.Sprintf("Reinstall beep version %s?", rel.Tag)
		}
	...
	fmt.Printf("%s %s (%s)\n",
		ui.Green("✓"),
		ui.Bold(fmt.Sprintf("Successfully upgraded beep to %s", res.NewVersion)),
